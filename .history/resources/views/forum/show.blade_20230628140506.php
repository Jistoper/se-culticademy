...
@section('content')
    <div class="max-w-7xl-auto py-6 sm:px-6 lg:px-8 bg-culti-green-4 items-center">
        <div class="sm:container sm:mx-auto px-6 rounded flex items-center justify-between mb-2">
            ...
            @if ((Auth::check() && Auth::id() == $forum->user_id) || (Auth::check() && Auth::id() == 1))
                                    ...
                                    <form action="{{ route('forum.update', $forum->id) }}" method="POST">
                                        ...
                                        <div class="-mx-3 md:flex mb-6">
                                            <div class="md:w-full px-3">
                                                ...
                                                <textarea style="height: 200px;" class="appearance-none block w-full bg-gray-100 text-gray-700 border @error('forum_desc') border-red-500 @enderror rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="forum_desc" placeholder="Enter forum description" name="forum_desc">{{ $forum->forum_desc }}</textarea>
                                                @error('forum_desc')
                                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                                @enderror
                                            </div>
